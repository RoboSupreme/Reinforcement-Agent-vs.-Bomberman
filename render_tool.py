    ...
    def __init__(self):
        ...
        self.standard = {
            ...
            "interactive": {
                ...
                # final explosion marker (game logic uses 6)
                6: ([255, 0, 0, 0.8 * 255], 0.7),
            }
        }
...
class RenderTool:
    ...
    def render_current_frame(self, save_media=True):
        ...
        ## foreground ##
        img_fg = None  # Initialize img_fg to ensure it's defined

        for obj_id in self.color_map["interactive"].keys():
            # Create a fresh transparent layer for the current obj_id
            # Using [0,0,0,0] for transparent black (alpha 0 makes color irrelevant)
            img_fg_pixels_for_current_obj = np.full(np.array(img_bg).shape, [0, 0, 0, 0], dtype='uint8')
            
            object_found_for_this_layer = False
            for r in range(self.game.board.shape[0]):
                for c in range(self.game.board.shape[1]):
                    # Safer access to movable_objects
                    if r in self.game.movable_objects and c in self.game.movable_objects[r]:
                        obj_set = self.game.movable_objects[r][c] # .copy() not needed for 'in' check
                        if obj_id in obj_set:
                            object_found_for_this_layer = True
                            # create tile
                            ...
                            # Modify img_fg_pixels_for_current_obj in place, no .copy() needed for the first argument
                            self.insert_tile(img_fg_pixels_for_current_obj, r, c, new_tile, size)
            
            if object_found_for_this_layer:
                img_fg_temp = Image.fromarray(img_fg_pixels_for_current_obj, 'RGBA')
                if img_fg is None:
                    img_fg = img_fg_temp # This is the first interactive layer
                else:
                    img_fg = Image.alpha_composite(img_fg, img_fg_temp) # Composite with previous layers

        # If no interactive objects were drawn at all, img_fg will still be None.
        # In this case, the final image is just the background.
        if img_fg is None:
            final_img = img_bg
        else:
